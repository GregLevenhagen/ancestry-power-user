...
async function markTheGroups(labelTitle) {
  ...
  for (let groupButton of groupButtons) {
    continueMarking = await new Promise((resolve, reject) => {
    ...
    if (!continueMarking) {
      logPort.postMessage({
        content: `total evaluated[${index}] added[${newlyChecked}]\nadding to group cancelled`,
      });
      return;
    }
    ...
    document.querySelectorAll(`.checkInputLabel`).forEach((label) => {
      ...
      if (label.title === labelTitle) {
        label.parentNode
          .querySelectorAll(`[id^='groupCallout_cbxTag']`)
          .forEach((chkbx) => {
            if (!chkbx.checked) {
              ...
              ++newlyChecked;
              logPort.postMessage({
                content: `added person[${index}] to group[${label.title}]`,
              });
            }
            if (index % 100 == 0 && index > 0) {
              logPort.postMessage({
                content: `added to group [${newlyChecked} / ${index}]`,
              });
            }
          });
      }
